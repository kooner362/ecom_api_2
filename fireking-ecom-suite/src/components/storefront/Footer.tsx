...
export default function StorefrontFooter() {
  ...
  const contactEmail = state.storeSettings?.email?.trim() || 'info@example.com';
    ...
    <footer className="bg-[hsl(220_22%_9%)] text-white/70">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="font-display font-700 text-white text-xl">
                {storeName}
                {storeHost ? <span style={{ color: state.theme.primaryColor }}> · {storeHost}</span> : null}
              </span>
            </div>
          ...
          <div>
            ...
            <p className="text-sm mb-4">Get exclusive deals and fireworks news delivered to your inbox.</p>
            {subscribed ? (
            ...
            <div className="mt-6 space-y-2">
              <div className="flex items-center gap-2 text-xs"><Mail className="w-3.5 h-3.5" /> {contactEmail}</div>
              {/* <div className="flex items-center gap-2 text-xs"><Phone className="w-3.5 h-3.5" /> 1-800-FIRE-KING</div> */}
      ...
      <div className="border-t border-white/8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs">© {currentYear} {storeHost || storeName} — All rights reserved.</p>
          <p className="text-xs">You must be 18+ to purchase fireworks. Use responsibly.</p>
        </div>
      </div>
    </footer>
  );
